...
const Deadend1 = () => {
  ...
  const handleOptionChange = async (value) => {
    ...
    if (value === 'love') {
      ...
      const response1 = await axios.patch('/attention/' + user.id, {
        attentionScore: 2,
      })
      const response2 = await axios.patch('/workEthic/' + user.id, {
        workEthicScore: 2,
      })
    }
    if (value === 'hate') {
            ...
            {showQuestion ? (
              <div className='flex p-2 items-center w-full justify-center h-12 mt-4 border-2  bg-[#111827]  rounded-md'>
                <div className='flex-none items-center justify-center'>
                  <p className='text-[#e2e8f0]'>Who is the killer ?</p>
                </div>
              </div>
            ) : (
              <div className='flex p-2 items-center w-full justify-center h-12 mt-4 border-2  bg-[#111827]  rounded-md'>
